...
public class MailBox : MonoBehaviour
{
    ...
    [Header("Sound")]
    [SerializeField] private float soundBoost = 0.5f;
    [SerializeField] AudioClip goodMail;
    [SerializeField] AudioClip badMail;
    ...
    void DrCatClick()
    {
        if (index == 0)
        {
            canvas.GetComponent<PopupManager>().enabled = true;
            audioSource.PlayOneShot(goodMail, soundBoost);
            Destroy(gameObject);
        }
        
        else audioSource.PlayOneShot(badMail, soundBoost);
    }
